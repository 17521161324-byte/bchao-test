...
async def start_test(
    ...
    result = await db.execute(
        ...
        .where(PatientRecord.record_id == data.record_id)
    ...
    asr_result = await db.execute(
        select(ModelConfig).where(ModelConfig.id == data.asr_model_id)
    )
    ...
    async def event_generator():
        ...
        try:
            ...
            # 推送完成事件
            yield f"event: complete\ndata: {json.dumps({'test_id': test_run.id, 'status': 'ok'}, ensure_ascii=False)}\n\n"
