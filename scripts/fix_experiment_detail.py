# Read current script-only content
    ...
    <a-tabs v-model:activeKey="activeTab" type="card">
      ...
      <a-tab-pane key="patients" tab="患者对比">
        <a-table :data-source="patientComparison" size="small" :scroll="{ x: 1800 }" row-key="key"
          ...
          <a-table-column title="左卵泡" :width="120"><template #default="{ record }"><div>金标: {{ record.gt_left ?? '-' }}</div><div :style="{ color: record.gt_left !== record.llm_left ? '#ff4d4f' : '#52c41a' }">LLM: {{ record.llm_left ?? '-', props: record.llm_left }}</div></template></a-table-column>
        </a-table>
      </a-tab-pane>
    </a-tabs>
...
print("Template prepended successfully")
